...
.kappagen {
  ...
  .scene .cube div[data-face] {
    height: var(--emote-height);
    width: var(--emote-width);
  }

  .scene .cube div[data-face] img {
    position: absolute;
    transform: translateZ(var(--cube-depth));
  }

  .scene .cube [data-face] {
    position: absolute;
    background-color: rgba(255, 255, 255, 0.2);
  }

  .scene .cube [data-face="front"] {
    transform: translateZ(var(--cube-depth));
  }

  .scene .cube [data-face="back"] {
    transform: rotateY(90deg) translateZ(var(--cube-depth));
  }

  .scene .cube [data-face="right"] {
    transform: rotateY(180deg) translateZ(var(--cube-depth));
  }

  .scene .cube [data-face="left"] {
    transform: rotateY(-90deg) translateZ(var(--cube-depth));
  }

  .scene .cube [data-face="top"] {
    transform: rotateX(90deg) translateZ(var(--cube-depth));
  }

  .scene .cube [data-face="bottom"] {
    transform: rotateX(-90deg) translateZ(var(--cube-depth));
  }
  ...
  .scene.cube:not(.kappa),
  .scene.cube:not(.kappa) .cube {
    transition: transform linear 5000ms;
  }
